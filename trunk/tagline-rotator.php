...
if
( !class_exists('WP_Tagline_Rotator') )
{
	class WP_Tagline_Rotator
	{
		...
		public function __construct()
		{
			...
			elseif
			(get_option( 'tagline_rotator_filter_wp_title' ))
			{
				add_action( 'wp_head', array( $this, 'add_filter' ) );
			}
			else
			{
		...
		{
			$taglines = get_option('tagline_rotator_taglines');
			$filter_wp_title = get_option('tagline_rotator_filter_wp_title');

			if
			( !empty($_POST) && check_admin_referer('tagline-rotator-update-options', 'tagline-rotator-nonce') )
			{
				...
				if
				(isset($_POST['new_tagline']) && $_POST['new_tagline'] !== '')
				{
					$new_tagline = wp_kses_post($_POST['new_tagline']);
					$taglines[] = $new_tagline;
				}

				//check whether we want to turn off the random tagline for wp_title
				if
				( isset( $_POST['filter_wp_title'] ) )
				{
					update_option( 'tagline_rotator_filter_wp_title', 1 );
					$filter_wp_title = 1;
				} else
				{
					delete_option( 'tagline_rotator_filter_wp_title' );
					$filter_wp_title = 0;
				}
...
?>
			...
			<div class="wrap">
				...
				<form method="post">
					<hr>
					<h3>Description Filters</h3>
					<table class="form-table">
						<tr valign="top">
							<th scope="row">Don't randomize tagline in header</th>
							<td><input type="checkbox" name="filter_wp_title" value="1" <?php checked( $filter_wp_title, 1 ); ?>></td>
						</tr>
					</table>
...
?>
						...
						<tr valign="top">
							...
							<td><input type="checkbox" name="box[<?php echo $key; ?>]" value="<?php echo $key;?>" /><input type="text" size="80" name="text[<?php echo $key; ?>]" value="<?php echo stripslashes($sanitized_tagline);?>" /><input type="hidden" name="vhtr_ids[]" value="<?php echo $key;?>" />
							</td>
						</tr>
